...
On Windows both produce `build/FreSH.exe` and `build/FreSH-Setup.exe`. The two
icons, the FreSH logo and the page that marks a `.frsh` script, are generated
by `tools/makeicon.c` and compiled into `FreSH.exe`; the installer embeds
`FreSH.exe` as a byte array generated by `tools/bin2c.c`, so the setup binary
is self contained.
