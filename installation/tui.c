...
void tui_screen(const char *title) {
    fputs("\x1b[2J\x1b[H\n", stdout);
    printf("  " ACCENT "FreSH" RESET " " DIM "%s" RESET "\n", FRESH_VERSION);
    printf("  %s\n", title);
    fputs("  ", stdout);
    rule();
    fputs("\n", stdout);
...
void tui_step(int ok, const char *message) {
    if (ok) printf("  " ACCENT "\xe2\x9c\x93" RESET " %s\n", message);
    else printf("  " ERROR "\xe2\x9c\x97" RESET " %s\n", message);
    fflush(stdout);
}
...
void tui_progress(int done, int total) {
    ...
    printf(RESET " %d%%", total > 0 ? done * 100 / total : 100);
    fflush(stdout);
}
...
int tui_menu(const char **options, int count) {
    ...
    while (1) {
        ...
        for (int i = 0; i < count; i++) {
            printf("\r\x1b[K");
            if (i == selected) printf("  " ACCENT "\xe2\x9d\xaf %s" RESET "\n", options[i]);
